...
class RiskManager:
    ...
    def record_exposure(self, cost: float, condition_id: str = "", question: str = "") -> None:
        ...
        log.info("Exposure updated: $%.2f / $%.2f (trade #%d)",
                 self.current_exposure, self.max_total_exposure, self.total_trades)
    ...
    def record_settlement(self, condition_id: str, payout: float) -> None:
        ...
        for pos in self.positions:
            if pos["condition_id"] == condition_id and not pos["settled"]:
                pos["settled"] = True
                profit = payout - pos["cost"]
                self.total_profit += profit
                self.current_exposure = max(0, self.current_exposure - pos["cost"])
    ...
    def settle_all_for_market(self, condition_id: str) -> float:
        ...
        if freed > 0:
            log.info("Freed $%.2f exposure from settled positions in %s", freed, condition_id[:20])
        return freed

    def can_trade(self) -> bool:
        """Check if we have headroom for any trade."""
        return self.current_exposure < self.max_total_exposure
    ...
    def summary(self) -> str:
        return (
            f"Exposure: ${self.current_exposure:.2f} / ${self.max_total_exposure:.2f} "
            f"| Per-trade cap: ${self.max_position_size:.2f} "
            f"| Available: ${self.max_allowed_cost():.2f} "
            f"| Trades: {self.successful_trades}ok/{self.failed_trades}fail "
